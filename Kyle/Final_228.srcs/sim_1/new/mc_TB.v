...
module mc_TB;
    ...
    reg [TB_BIT_WIDTH-1:0] Ai_TB, Bi_TB, Ci_TB;
    reg ENM_TB, ENA_TB;
    reg CLK_TB;
    wire [TB_BIT_WIDTH-1:0] Ao_TB, Bo_TB, Co_TB;
    wire OF_TB;
    ...
    reg [TB_BIT_WIDTH:0] t;
    reg [TB_BIT_WIDTH:0] i;
    initial
    begin
        ...
        ENA_TB <= 0;
        t <= {TB_BIT_WIDTH{1'b0}};
        for(i = {TB_BIT_WIDTH{1'b0}}; i < { 2'b11,{TB_BIT_WIDTH-1{1'b0}} }; i = i + {{TB_BIT_WIDTH{1'b0}}, 1'b1})
        begin
            Ai_TB <= i[TB_BIT_WIDTH-1:TB_BIT_WIDTH/2];
            Bi_TB <= i[TB_BIT_WIDTH/2 -1:0];
            #5;
            $display("t%d | %d %d %d | %d %d %d | %b |", t, Ai_TB, Bi_TB, Ci_TB, Ao_TB, Bo_TB, Co_TB, OF_TB); //like printf() in the C programming language
            #5;
            t <= t + { {TB_BIT_WIDTH-1{1'b0}}, 1'b1 };
        end
        $finish;
    end
endmodule
